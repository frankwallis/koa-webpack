...
module.exports = function(compiler, options) {
	...
	function* middleware(next) {
		...
		var filename = getOutputFilename(this.path);
		if (!filename) return yield next;
	...
	function getOutputFilename(filename) {
		...
		/* if it's not in public path then it's not a webpack file */
		if (filename.indexOf(options.publicPath) !== 0)
			return false;

		/* return the filename relative to the public path */
		return filename.slice(options.publicPath.length -1);
	}
